...
%{
    ...
    Também foi estudado o comportamento do sistema para uma amostra de combinações de níveis de abundância, ilustrando assim o desempenho do modelo.
...
%{
Após se ter carrgado o ficheiro: presas.mat, por aproximação das variáveis N_2(0) e \alfa2 gerou-se um aproximação dos resultados obtidos carregados.
%}
delta1 = 3.1;
alfa1 = 1.4;
N1o = timeseries(4);
...
%{
    De acordo com o estudado na pergunta 2.3, cada solução corresponde a um par de condições iniciais. Assim, a unicidade da solução aproximada está garantida.
%}
...
%{  
    Através da função fminsearch é possível obter o mínimo de uma função
    sem recurso à derivada da mesma, este método permite-nos obter um valor
    muito próximo ou mesmo o ótimo mínimo global da função.
%}
